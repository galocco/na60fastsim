...
void GenerateSignalCandidates(Int_t nevents = 10000, 
  ...
  if (writeNtuple)
  {
    ...
    if(!dataForFit){
      ntgen = new TNtuple("ntgen", "ntgen", "pt", 32000); 
    }
    if(nbody == 2)
      ntcand = new TNtuple("ntcand", "ntcand", "m:pt:rapidity:dist:cosp:d0prod:ptMin:ptMax:dca:thetad:arm:qt", 32000);
    else
      ntcand = new TNtuple("ntcand", "ntcand", "m:pt:rapidity:dist:distD:cosp:cospD:bxy:bxyD:dca:dcaD:arm:armD:qt:qtD:mD", 32000);
  }

  Float_t mD = 0;
  Float_t* arrnt = new Float_t[(nbody == 2) ? 12 : 16];
  Float_t arrntgen[4];
      ...
      if (IsDecayDaughter && IsStable){
        ...
        if(IsReconstructable && nrec==0){
          if (!dataForFit){
            arrntgen[0] = ptGen;
            /*
            if(TMath::Sqrt(pdgParticle)!=3334){
              arrntgen[1] = yGen;
...
void MakeCombinBkgCandidates3Body(const char* trackTreeFileBkg="treeBkgEvents_layer5.root",
  ...
  Double_t massDau[3];
  Double_t massDau_range_min = massUnstable-0.001*5;
  Double_t massDau_range_max = massUnstable+0.001*5;
  massDau[0] = TDatabasePDG::Instance()->GetParticle(pdgDaughter[0])->Mass();
  massDau[1] = TDatabasePDG::Instance()->GetParticle(pdgDaughter[1])->Mass();
  massDau[2] = TDatabasePDG::Instance()->GetParticle(pdgDaughter[2])->Mass();
  std::cout<<"pdg_dau0: "<<pdg_dau[0]<<"\n";
  std::cout<<"pdg_dau1: "<<pdg_dau[1]<<"\n";
  std::cout<<"pdg_dau0: "<<pdg_dau2[0]<<"\n";
  std::cout<<"pdg_dau1: "<<pdg_dau2[1]<<"\n";
  ...
  Float_t  arrnt[nAxes+2];
  if (writeNtuple){
    fnt = new TFile(Form("fntBkg%s.root",suffix.Data()), "recreate");
    ntcand = new TNtuple("ntcand", "ntcand", "m:pt:rapidity:dist:distD:cosp:cospD:bxy:bxyD:dca:dcaD:mD:arm:armD:qt:qtD:true", 32000);                                    
  }
      ...
      for (Int_t itr2 = itr+1; itr2 < arrentrBkg+arrentrSig; ++itr2){
        ...
        for (Int_t itr3 = itr2+1; itr3 < arrentrBkg+arrentrSig; ++itr3){
          ...
          for(Int_t iMassHyp=0; iMassHyp<2; iMassHyp++){
            ...
            recProbe[1].GetPXYZ(pxyz); //particle with ch*chMother < 0
            daurec[1].SetXYZM(pxyz[0], pxyz[1], pxyz[2], massDau[1]);
            recProbe[2*iMassHyp].GetPXYZ(pxyz); //particle with ch*chMother > 0
            daurec[2*iMassHyp].SetXYZM(pxyz[0], pxyz[1], pxyz[2], massDau[2]);
            parentD = daurec[1];
            parentD += daurec[2*iMassHyp];

            TVector3 sameSignD(daurec[2*iMassHyp].Px(),daurec[2*iMassHyp].Py(),daurec[2*iMassHyp].Pz());  //negative if chMother is negative
            ...
            if(massPair < massDau_range_min || massPair > massDau_range_max) continue;
            if(chMother < 0)
              armD = ArmPod(oppSignD, sameSignD, momD, qTD);
            else
              armD = ArmPod(sameSignD, oppSignD, momD, qTD);
            
            if(IsNotOmega && pdgMother*armD < 0) continue;
            ...
            daurec[2*(1-iMassHyp)].SetXYZM(pxyz[0], pxyz[1], pxyz[2], massDau[0]);
            helper.GetPXYZ(pxyz);
            daurec[3].SetXYZM(pxyz[0], pxyz[1], pxyz[2], massUnstable);
            parent = daurec[2*(1-iMassHyp)];
            parent += daurec[3];

            Double_t cosp = CosPointingAngle(vprim, VSec, parent);
            //skip if the cosine of the pointing angle is too small          
            if(cosp < 0.9999) continue;
            ...
            if(dist > distPair) continue;
            
            TVector3 sameSign(daurec[2*(1-iMassHyp)].Px(),daurec[2*(1-iMassHyp)].Py(),daurec[2*(1-iMassHyp)].Pz()); 
            ...
            if(IsNotOmega && pdgMother*arm > 0) continue;
            ...
            if(invMass<mass_range_min  || invMass>mass_range_max) continue;
            countCandInPeak++;
            ...
            if (d0x < 0)
              bxyD *= -1;
            if(itr<arrentrBkg || itr2<arrentrBkg || itr3<arrentrBkg)
              trueCand = 0;
            else
              trueCand = IsTrueCandidate(recProbe, pdgMother, daurec, 3, iMassHyp);
            hCosp->Fill(cosp, pt);
...
void MakeCombinBkgCandidates2Body(const char* trackTreeFileBkg="treeBkgEvents_layer5.root",
  ...
  TNtuple *ntcand = 0x0;
  Double_t arrhsp[8];
  Float_t  arrnt[IsPhi ? 13 : 10];

  if (writeNtuple){
    fnt = new TFile(Form("fntBkg%s.root",suffix.Data()), "recreate");
    if(pdgMother!=333)
      ntcand = new TNtuple("ntcand", "ntcand", "m:pt:rapidity:dist:cosp:d0prod:dca:arm:qt:true", 32000);
    else
  ...
  for (Int_t iev = 0; iev < nevents; ++iev){
    ...
    for (Int_t itr = 0; itr < arrentrBkg+arrentrSig; ++itr){
      ...
      for (Int_t itr2 = itr+1; itr2 < arrentrBkg+arrentrSig; ++itr2){ 
        ...
        float qT;
        arm = ArmPod(plus, minus, mom, qT);
        if(IsLambda && pdgMother*arm<0) continue;
        
        Float_t dist = TMath::Sqrt(vsec[0] * vsec[0] + vsec[1] * vsec[1] + vsec[2] * vsec[2]);
        if(dist > 0.1 && IsPhi) continue;
        ...
        Int_t trueCand = IsTrueCandidate(recProbe, pdgMother, daurec, 2);
        hYPtReco->Fill(y, pt);
        hMass->Fill(invMass);
        hDCA->Fill(dca, pt);
        hDCAx->Fill(d1, pt);
        ...
        if (writeNtuple){
          ...
          if(IsPhi){
            ...
            //arrnt[12] = recProbe[1].GetNITSHits();
            //arrnt[13] = recProbe[0].GetNITSHits();
            arrnt[12] = trueCand;
          }
          else{
            arrnt[9] = trueCand;
