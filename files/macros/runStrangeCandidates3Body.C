...
const int NEnergy3 = 5;
double Elab3[NEnergy3] = {20,30,40,80,158};
double Tslope3[2][NEnergy3] = {{196.8,237.4,244.6,239.8,298.7},{0,0,244.6,239.8,298.7}};//{0,0,218,0,267},
double sigma_rapidity3[2][NEnergy3] = {{0.425,0.538,0.696,0.658,1.2},{0,0,0.694,0.742,0.839}};
double y0_rapidity3[2][NEnergy3] = {{0.425,0.538,0.487,0.682,1.451},{0,0,0.725,0.792,0.88}};
double multiplicity3[2][NEnergy3] = {{0,0,0,0,100},{0,0,59.1,76.9,103.0}};
//double pdg_code[2][5] = {{3334,3122,321,2212,-211},{310,211,-211,0,0}};
double pdg_code3[2] = {3334,310};//3334,421};//

double GetTslope3(int pdgParticle, double Eint);
double GetSigmaRapidity3(int pdgParticle, double Eint);
double GetY0Rapidity3(int pdgParticle, double Eint);
double GetMultiplicity3(int pdgParticle, double Eint);
double GetY03(double Eint);
const int nEv = 10000;

void GenerateStrangeSignalCandidates3Body(Int_t nevents = 100000, 
				double Eint = 158., 
				const char *setup = "../setups/setup_short_5pixel_1.5T.txt",
				const char *privateDecayTable = "../decaytables/USERTABOMEGA.DEC",
				int optPartAntiPart=3,
				bool writeNtuple = kTRUE, 
				bool simulateBg=kTRUE,
        int pdgParticle=3334){
  ...
  fpt->SetParameter(1,GetTslope3(pdgParticle,Eint));
  TF1 *fy = new TF1("fy","expo(-0.5*((x-[0])/[1])**2)+expo(-0.5*((x+[0])/[1])**2) ",yminSG,ymaxSG);
  fy->SetParameter(0,GetY0Rapidity3(pdgParticle,Eint));
  fy->SetParameter(1,GetSigmaRapidity3(pdgParticle,Eint));
  ...
  TH1D* hMassAll = new TH1D("hMassAll", "Mass all match", 200, 1., 10);
  TH1D* hMassFake = new TH1D("hMassFake", "Mass fake match", 200, 1., 10);
  ...
  if (writeNtuple){
    fnt = new TFile("fntSig3body.root", "recreate");
  ...
  for (Int_t iev = 0; iev < nevents; iev++){
    ...
    Double_t ptGenD = fpt->GetRandom(); // get Ds distribution from file
    Double_t yGenD = fy->GetRandom()+GetY03(Eint);
    ...
    //std::cout<<nrec<<" "<<nPions<<" "<<nKaons<<std::endl;
...
double GetTslope3(int pdgParticle, double Eint){
  ...
  return Tslope3[index_pdg][index_E];
}
...
double GetSigmaRapidity3(int pdgParticle, double Eint){
  ...
  return sigma_rapidity3[index_pdg][index_E];
}
...
double GetY0Rapidity3(int pdgParticle, double Eint){
  ...
  return y0_rapidity3[index_pdg][index_E];
}
...
double GetMultiplicity3(int pdgParticle, double Eint){
  ...
  return multiplicity3[index_pdg][index_E];
}
