...
TH2D* fill_hist(TString filename, Double_t cuts[2][]);
...
TH2D* fill_hist(TString filename, Double_t cuts[2][]){
    ...
    THnSparse* hsp = (TTree*)input->Get("hsp");
    int naxis = hsp->GetNdimensions();
    for(int ax = 0; ax < naxis; ax++){
        int bin_min = hsp->GetAxis(ax).FindBin(cuts[0][ax])
        int bin_max = hsp->GetAxis(ax).FindBin(cuts[0][ax])
        hsp->GetAxis(ax)->SetRange(bin_min, bin_max);
    }
    TH2D* hist = hsp.Projection(0, 1);
...
/*
std::vector<TString> files_list(TString dirname="C:/root/folder/", TString ext=".root"){
